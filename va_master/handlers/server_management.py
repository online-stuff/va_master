...
#Kwargs typically has various data required to access the server
#It is mostly used for non-salt apps, where we need data such as usernames, passwords etc. 
@tornado.gen.coroutine
def manage_server_type(datastore_handler, server_name, new_type, ip_address = None, username = None, driver_name = None, provider_name = None, role = None, app_type = 'salt', kwargs = {}):
    ...
    if new_type == 'ssh':
        ...
        server = yield handle_app(datastore_handler, server = server, role = role, app_type = app_type)
